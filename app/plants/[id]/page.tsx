...
export default function PlantPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchPlantData = async () => {
      ...
      try {
        // Fetch plant details
        const plantRef = doc(db, "users", userId, "plants", params.id)
        ...
        // Fetch logs
        const logsRef = collection(db, "users", userId, "plants", params.id, "logs")
        ...
        const envRef = collection(db, "users", userId, "plants", params.id, "environment")
        ...
        // Fetch photos
        const photosRef = collection(db, "users", userId, "plants", params.id, "photos")
  ...
  }, [userId, params.id, toast, t, router])
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `users/${userId}/plants/${params.id}/photos/${Date.now()}_${file.name}`)

      await uploadBytes(storageRef, file)
      const downloadURL = await getDownloadURL(storageRef)

      // Save photo reference to Firestore
      const photoRef = collection(db, "users", userId, "plants", params.id, "photos")
    ...
    return (
      <Layout>
        <Card>
          ...
          <CardContent>
            <Button onClick={() => router.push("/dashboard")}>{t("plantPage.backToDashboard")}</Button>
          </CardContent>
        </Card>
      </Layout>
    )
      ...
      <Tabs defaultValue="details" className="w-full">
        <TabsList className="grid grid-cols-4 mb-6">
          ...
          <TabsTrigger value="environment">
            ...
            <span className="hidden sm:inline">{t("plantPage.environment")}</span>
          </TabsTrigger>
        ...
        <TabsContent value="journal">
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              ...
              <CardContent>
                <AddLogForm plantId={params.id} onSuccess={(newLog) => setLogs([newLog, ...logs])} />
              </CardContent>
            </Card>
        ...
        <TabsContent value="environment">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("plantPage.environmentDataDesc")}</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="photos">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("plantPage.photoGalleryDesc")}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="mb-4">
              ...
              <PhotoGallery photos={photos} plantId={params.id} />
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
