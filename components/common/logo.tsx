 ...
 * - Based on a minimal leaf/sprout silhouette
 * - Slightly rotated and centered to differentiate from the original source
 * - Uses currentColor by default so it adapts to text color (e.g. text-primary)
 */
export function Logo({
  ...
  // Badge variants: add a rounded background container
  const isDark = variant === "badgeDark";
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 20 20"
      ...
      height={dim}
      role="img"
      aria-label="Cannafriend logo badge"
      className={cn("inline-block", className)}
      style={style}
      {...props}
    >
      ...
      />
      {/* center the 14x14 mark within 20x20 */}
      <g transform="translate(3,3)" fill={isDark ? "#10B981" : "#10B981"}>
        {Mark}
      </g>
    </svg>
  );
