...
export function JournalEntries({ logs }: JournalEntriesProps) {
  ...
  const getLogTitle = (log: LogEntry) => {
    switch (log.type) {
      case "watering":
        return `${t("logType.watering")} - ${log.amount}ml (${t(`watering.${log.method}`)})`
      case "feeding":
        return `${t("logType.feeding")} - ${log.npk} (${log.amount}ml/L)`
      case "training":
        return `${t("logType.training")} - ${t(`training.${log.method}`)}`
      case "environment":
        return `${t("logType.environment")} - ${log.temperature}°C, ${log.humidity}%, pH ${log.ph}`
      default:
    ...
    <div className="space-y-4">
      {logs.map((log) => (
        <div key={log.id} className="border-b pb-4 last:border-0 last:pb-0">
          ...
          <div className="text-xs text-muted-foreground mb-1">{log.date && format(parseISO(log.date), "PPP p")}</div>
